...
```bash
GET /api/v1/semantic-search?project=gca&q=authentication logic&k=10
```
...
- `GET /api/v1/predicates` - Get graph schema

**Querying**
- `POST /api/v1/query` - Execute Datalog queries
- `GET /api/v1/symbols` - Fuzzy symbol search
- `GET /api/v1/semantic-search` - Vector similarity search

**Graph Exploration**
- `GET /api/v1/graph/map` - File-level overview
- `GET /api/v1/graph/file-details` - Symbol-level details
- `GET /api/v1/graph/backbone` - Cross-file architecture
- `GET /api/v1/graph/path` - Shortest path between symbols
- `GET /api/v1/graph/subgraph` - Retrieve specific subgraph for cluster expansion
...
**AI Integration**  
- `POST /api/v1/ai/ask` - Natural language queries with context injection
...
**Example:**
```bash
curl 'http://localhost:8080/api/v1/semantic-search?project=gca&q=graph%20pathfinding&k=5'
```
...
**Example:**
```bash
curl 'http://localhost:8080/api/v1/graph/path?project=gca&source=main.go:main&target=handlers.go:handleQuery'
```
...
**Endpoint:** `GET /api/v1/graph/subgraph`
...
**Example:**
```bash
curl 'http://localhost:8080/api/v1/graph/subgraph?project=gca&nodes=node1,node2,node3'
```
