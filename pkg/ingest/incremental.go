...
func removeDeletedFiles(s *meb.MEBStore, projectName string, deletedFiles []string) {
	for _, filePath := range deletedFiles {
		...
		s.Query(context.Background(), fmt.Sprintf(`triples(%s, ?p, ?o)`, quotedPath))
		s.Query(context.Background(), fmt.Sprintf(`triples(?s, ?p, %s)`, quotedPath))

		log.Printf("Marked %s as deleted", filePath)
	}
}
