...
func transformToGraph(results []map[string]any) GraphResponse {
    ...
    for _, row := range results {
        ...
        if s, ok := row["S"]; ok { subj = fmt.Sprint(s) }
        else if s, ok := row["Subject"]; ok { subj = fmt.Sprint(s) }
        
        if p, ok := row["P"]; ok { pred = fmt.Sprint(p) }
        else if p, ok := row["Predicate"]; ok { pred = fmt.Sprint(p) }
        
        if o, ok := row["O"]; ok { obj = fmt.Sprint(o) }
        else if o, ok := row["Object"]; ok { obj = fmt.Sprint(o) }
...
func readFileRange(path string, start, end int) (string, error) {
    ...
    if start < 1 { start = 1 }
    if end == -1 || end > len(lines) { end = len(lines) }
    
    // Adjust to 0-indexed slice
    if start > len(lines) { return "", nil }
