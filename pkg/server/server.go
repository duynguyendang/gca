...
func NewServer(mgr *manager.StoreManager, sourceDir string) *Server {
	...
	s := &Server{
		...
		reportService:  service.NewReportService(mgr, service.NilNarrative(aiSvc)),
		impactService:  service.NewImpactReportService(ephemeralStore, mgr, service.NilNarrative(aiSvc)),
		aiService:      aiSvc,
